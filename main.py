...
# All field names (FileNo, ArabicName, etc.) mirror the real API
# ─────────────────────────────────────────────────────────────

# PATIENTS — exactly as returned by real CRM (only these 5 fields)
patients = [
    {
        "FileNo": "11763",
        "ArabicName": "أحمد محمد السالم",
        "EnglishName": "Ahmed Mohamed AlSalem",
        "Mobile": "0538892874",
        "IDCard": "1098765432"
    },
    {
        "FileNo": "7855",
        "ArabicName": "فاطمة عبدالله الزهراني",
        "EnglishName": "Fatima Abdullah AlZahrani",
        "Mobile": "0501234567",
        "IDCard": "2087654321"
    },
    {
        "FileNo": "9240",
        "ArabicName": "محمد علي الغامدي",
        "EnglishName": "Mohammed Ali AlGhamdi",
        "Mobile": "0556789012",
        "IDCard": "1076543210"
    },
    {
        "FileNo": "12316",
        "ArabicName": "أنس محمد احمد",
        "EnglishName": "Anas Mohamed Ahmed",
        "Mobile": "0512345678",
        "IDCard": "2134567892"
    }
]

# SPECIALTIES — EXACT copy from real Dentak CRM (21 specialties)
specialties = [
    ...
    {"Id": 6,  "NameArabic": "الباطنة",                             "NameEnglish": "الباطنة"},
...
# DOCTORS — Real doctors from specialty 10 (Dental & Orthodontics) are EXACT from real CRM.
# Doctors for other specialties are mock but use the same real field structure.
doctors = [
    # ── Specialty 10 — الأسنان والتقويم (REAL from CRM) ──────────────────
    ...
    # ── Other specialties — mock doctors with real field structure ─────────
...
# DOCTOR SCHEDULES — EXACT real CRM format:
# KEY FACTS from real CRM:
#   1. Each entry has "DoctorId" INSIDE the object (not just as the dict key)
#   2. StartTime = EndTime — both hold the SESSION START TIME (real CRM quirk, not a range)
#   3. SlotMinutes = 15 (matches real CRM)
#   4. One doctor can have multiple entries per day = multiple sessions that day
doctor_schedules = {
    5: [   # د. عبدالرحمن قنوت — Specialty 10
    ...
    7: [   # د. هنادي الحارثي — Specialty 10
    ...
    9: [   # د. ولاء عبداللطيف — Specialty 10
    ...
    10: [  # د. عبدالعزيز البارقي — Specialty 10
    ...
    77: [  # Dr. Ahmed ElSayed — Specialty 10
    ...
    # ── Other specialty doctors (mock but same real format) ───────────────
    20: [  # د. سارة الشمري — Dermatology
    ...
    51: [  # د. لمياء القحطاني — Pediatrics
    ...
    70: [  # د. أحمد الحربي — Internal Medicine
...
# INSURANCE DATA per patient FileNo
insurance_data = {
    ...
    "12316": []   # أنس محمد احمد — no insurance registered
}

# APPOINTMENTS — real CRM field names
appointments = [
    {
        "_internal_id": 46577,
        "AppointmentId": "46577",
        ...
        "StartTime": "10:00 am",
        "EndTime": "10:00 am",   # real CRM: EndTime = StartTime
        "Status": "Confirmed",
        "patient_name": "أحمد محمد السالم",
        "doctor_name": "د. عبدالرحمن قنوت",
        "date": "2026/03/10",
        "fromtime": "10:00 am",
        "endtime": "10:00 am",
        "status": "confirmed"
...
def generate_slots(doctor_id, date_str, schedule_entries):
    """
    Generate available slots matching real CRM format exactly:
    - [{DocotorID, SlotDate, StartTime, EndTime}]  (typo "DocotorID" intentional — real CRM)
    - StartTime = EndTime (both hold the slot start time — real CRM quirk)
    - 15-minute slots generated from each session's StartTime for 2 hours

    schedule_entries: list of schedule rows for THIS day (already filtered by day_id)
    """
    ...
    # Collect already-booked start times for this doctor+date
    ...
    for entry in schedule_entries:
        ...
        # Generate slots for 2 hours from the session start
        session_end = session_start + td(hours=2)
        current = session_start
        while current < session_end:
            time_str = current.strftime("%I:%M %p").lower().lstrip("0")
            # Ensure leading zero for single-digit hours to match CRM format (e.g. "09:00 am")
            time_str_padded = current.strftime("%I:%M %p").lower()

            if time_str_padded not in booked_times:
                all_slots.append({
                    "DocotorID": str(doctor_id),   # typo intentional — matches real CRM
                    "SlotDate":  date_str,
                    "StartTime": time_str_padded,   # e.g. "10:00 am"
                    "EndTime":   time_str_padded    # same as StartTime — real CRM format
                })
...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .Cancelled,.cancelled { background:#dc262622; color:#f87171; border:1px solid #f8717144; }
    .Rescheduled,.rescheduled { background:#7c3aed22; color:#a78bfa; border:1px solid #a78bfa44; }
    .Booked,.booked { background:#0369a122; color:#38bdf8; border:1px solid #38bdf844; }
...
async function load() {
  try {
    ...
    document.getElementById('stats').innerHTML =
      ...
      stat(appts.filter(a => (a.Status||a.status||'').toLowerCase() === 'confirmed').length, 'مواعيد مؤكدة') +
      stat(appts.filter(a => (a.Status||a.status||'').toLowerCase() === 'cancelled').length, 'مواعيد ملغاة') +
      stat(appts.filter(a => (a.Status||a.status||'').toLowerCase() === 'held').length, 'محجوزة مؤقتاً')

    document.getElementById('appt-body').innerHTML = appts.length
      ? appts.map(a => `<tr>
          <td><strong>#${a.AppointmentId || a._internal_id}</strong></td>
          <td>${a.PatientName || a.patient_name || '—'}</td>
          <td>${a.DoctorName  || a.doctor_name  || '—'}</td>
          <td>${a.AppointmentDate || a.date || '—'}</td>
          <td>${a.StartTime || a.fromtime || '—'} → ${a.EndTime || a.endtime || '—'}</td>
          <td><span class="badge ${a.Status || a.status}">${a.Status || a.status}</span></td>
        </tr>`).join('')
...
async function resetData() {
  if (!confirm('مسح المواعيد والملاحظات؟ (المرضى لن يُمسحوا)')) return
...
# ─────────────────────────────────────────────────────────────
# INTERNAL APIs (dashboard use only)
# ─────────────────────────────────────────────────────────────
...
def api_patients():
    # add insurance flag for display
    result = []
    for p in patients:
        row = dict(p)
...
@app.route('/api/debug')
def api_debug():
    return jsonify({"last_20_logs": activity_log[:20], "total_appointments": len(appointments), "total_patients": len(patients)})
...
# ─────────────────────────────────────────────────────────────
# PATIENT ENDPOINTS — Real CRM format
# ─────────────────────────────────────────────────────────────

@app.route('/MyCallAi/patients/search')
def search_patient():
    """
    Real CRM returns: ARRAY of patient objects using FileNo, ArabicName, etc.
    Empty array = patient not found.
    """
    phone = request.args.get('phone', '').strip()
    patient = find_patient_by_mobile(phone)
    if patient:
        log(f"🔍 تم إيجاد المريض: {patient.get('ArabicName')} ({phone})")
        return jsonify([patient])   # ← ARRAY, exactly like real CRM
    else:
        log(f"🔍 لم يتم إيجاد المريض بالرقم: {phone}")
        return jsonify([])          # ← Empty ARRAY = not found (NOT 404)


@app.route('/MyCallAi/patients/<pid>')
def get_patient(pid):
    """Returns single patient object by FileNo."""
...
def create_patient():
    ...
    """
    data = request.get_json() or {}
    new_id = str(int(datetime.now().timestamp()))[-5:]  # 5-digit ID
    new_patient = {
        "FileNo": new_id,
    ...
    return jsonify({"Status": "SUCCESS", "id": new_id})  # ← Real CRM format


@app.route('/MyCallAi/patients/<pid>/insurance')
def patient_insurance(pid):
    """
    Real CRM returns: {patientId, count, data: [{InsuranceCompany, PolicyNumber, ...}]}
    """
...
def get_patient_appointments(pid):
    """
    CUSTOM endpoint — not in original Dentak API yet (requested).
    Returns all appointments for a patient by FileNo.
    """
    patient_appts = [a for a in appointments if str(a.get('PatientId')) == str(pid)]
    log(f"📋 تم جلب مواعيد المريض #{pid} — عدد: {len(patient_appts)}")
    return jsonify({
        "patientId": str(pid),
...
# ─────────────────────────────────────────────────────────────
# SPECIALTIES & DOCTORS — Real CRM format
# ─────────────────────────────────────────────────────────────

@app.route('/MyCallAi/specialties')
def get_specialties():
    """
    Real CRM returns: {count, data: [{Id, Code, NameArabic, NameEnglish}]}
    """
...
def get_doctors():
    """
    Real CRM returns: {specialtyId, count, data: [{Id, ArabicName, EnglishName, SpecialtyId, SpecialtyName}]}
    """
    ...
    if specialty_id:
        ...
        result = doctors
    log(f"👨‍⚕️ تم جلب قائمة الأطباء (تخصص: {specialty_id or 'الكل'})")
    return jsonify({
        "specialtyId": specialty_id or "all",
        "count": len(result),
        "data": result
    })
...
def get_schedule(did):
    """
    Real CRM returns: {doctorId, count, data: [{DoctorId, DayOfWeek, DayName, StartTime, EndTime, SlotMinutes}]}
    Each entry has DoctorId INSIDE it. StartTime = EndTime (session start time).
    """
    doctor = find_doctor(did)
    schedule = doctor_schedules.get(did, [])
    log(f"🗓️ جدول الطبيب: {doctor['ArabicName'] if doctor else f'#{did}'} — {len(schedule)} جلسة")
    return jsonify({
        "doctorId": did,
        "count": len(schedule),
        "data": schedule
...
# ─────────────────────────────────────────────────────────────
# APPOINTMENT ENDPOINTS — Real CRM format
# ─────────────────────────────────────────────────────────────

@app.route('/MyCallAi/appointments/available', methods=['GET', 'POST'])
def get_available():
    """
    Real CRM returns: [{DocotorID, SlotDate, StartTime, EndTime}]
    StartTime = EndTime (both = slot start time — real CRM format)
    """
    ...
    date_fmt  = data.get('app_dt_fmt')   # YYYYMMDD integer
    ...
    if doctor_id and day_id:
        # Get schedule entries for this doctor on this specific day
    ...
    log(f"🕐 مواعيد متاحة — طبيب {doctor_id}, تاريخ {date_str}, اليوم {day_id} → {len(slots)} موعد")
...
def create_appointment():
    """Direct book (no hold). Real CRM returns: {"Status": "SUCCESS", "id": "48504"}"""
    data = request.get_json() or {}
    patient = find_patient_by_fileno(data.get('patientid', ''))
    doctor  = find_doctor(data.get('doctorid', 0)) if data.get('doctorid') else None
    new_id  = str(int(datetime.now().timestamp()))
...
def get_appointment(aid):
    """
    Real CRM returns: {AppointmentId, PatientId, PatientName, PatientMobile,
                        DoctorId, DoctorName, AppointmentDate, StartTime, EndTime, Status}
    """
...
def confirm_appointment(aid):
    """
    Real CRM returns: plain text string "Appointment is Confirmed"
    """
    global appointments
    found = False
    for i, a in enumerate(appointments):
        if str(a.get('AppointmentId')) == str(aid) or str(a.get('_internal_id')) == str(aid):
            ...
            break
    if not found:
        log(f"⚠️ محاولة تأكيد موعد غير موجود: #{aid}")
    return "Appointment is Confirmed", 200, {'Content-Type': 'text/plain'}  # ← Real CRM format


@app.route('/MyCallAi/appointments/<aid>/cancel', methods=['POST'])
def cancel_appointment(aid):
    """
    Real CRM returns: plain text string "Appointment is Canceled"
    """
    global appointments
    found = False
    for i, a in enumerate(appointments):
        if str(a.get('AppointmentId')) == str(aid) or str(a.get('_internal_id')) == str(aid):
            ...
            break
    if not found:
        log(f"⚠️ محاولة إلغاء موعد غير موجود: #{aid}")
    return "Appointment is Canceled", 200, {'Content-Type': 'text/plain'}   # ← Real CRM format


@app.route('/MyCallAi/appointments/<aid>/reschedule', methods=['POST'])
def reschedule_appointment(aid):
    """
    Real CRM returns: plain text string "Appointment is Rescheduled"
    """
    global appointments
    data = request.get_json() or {}
    found = False
    for i, a in enumerate(appointments):
        if str(a.get('AppointmentId')) == str(aid) or str(a.get('_internal_id')) == str(aid):
            ...
            if data.get('appointmentdate'):
                ...
                appointments[i]['date'] = data['appointmentdate']
            if data.get('fromtime'):
                appointments[i]['StartTime'] = data['fromtime']
                appointments[i]['fromtime'] = data['fromtime']
            if data.get('endtime'):
    ...
    return "Appointment is Rescheduled", 200, {'Content-Type': 'text/plain'}  # ← Real CRM format
...
def send_message():
    data = request.get_json() or {}
    log(f"💬 رسالة إلى {data.get('to', '—')} — نوع: {data.get('type', 'WhatsApp')}")
    return jsonify({"message_id": str(int(datetime.now().timestamp())), "status": "sent"})
...
def whatsapp_webhook():
    data = request.get_json() or {}
    log(f"📲 رسالة واردة على واتساب من {data.get('from', '—')}")
